...
Scrollbar.init(document.querySelector(".content-playlist"));
...
const playbuttons = document.querySelectorAll("[song]");
...
playbuttons.forEach((e) => {
	e.addEventListener("dblclick", function () {
		const songname = e.getAttribute("song");
		const howlToPlay = howls.find((howl) => howl._src === playlist.find((track) => track.songName === songname).url);
		...
		if (howlToPlay) {
			...
			currentTrackIndex = howls.indexOf(howlToPlay);
		}
	});

	const play = e.querySelector(".song-play-button");
...
// function changeplayButtoninfo() {
// 	let buttons = document.querySelectorAll("[song]");
// 	buttons.forEach((e) => {
